<template>
  <div>
    ...
    <div class="px-6">
      ...
      <InputPopUp v-model="confirmPassword" v-if="popUpOpen" title="Authentication required"
                  text="Please enter your password to change your email" type="password" button="Change email"
                  @cancel="popUpOpen = false" @confirm="changeEmail"/>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    async changeEmail(password: string) {
      await updateEmail(this.email, password);
    },
    async changeName() {
